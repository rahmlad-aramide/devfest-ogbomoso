...
//       // Check if header is over a white background section
...
//                 key={item.label}
//                 href={item.href}
//                 className="relative text-white/90 hover:text-white transition-all duration-300 font-medium text-sm lg:text-base px-5 py-2.5 rounded-full bg-white/10 backdrop-blur-md border border-white/20 overflow-hidden
    ...
    const handleScroll = () => {
      ...
      sections.forEach((section) => {
        ...
        if (rect.top <= headerHeight && rect.bottom >= 0) {
          ...
          // This checks for white, near-white, and light colors
          const rgb = bgColor.match(/\d+/g);
          if (rgb) {
            const [r, g, b] = rgb.map(Number);
            const brightness = (r * 299 + g * 587 + b * 114) / 1000;
